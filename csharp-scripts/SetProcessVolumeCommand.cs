...
public class Startup {
    public async Task<object> Invoke(object input) {
        ...
        if (args.Length < 3) {
            Console.WriteLine("Not enough args passed to setprogramvolume!");
            return null;
        }

        string pidStr = args[1]; // The program to set the volume for
        string volStr = args[2]; // Should be in the range [0, 100]
        int pid;
        short volume;

        if (Int32.TryParse(pidStr, out pid)) {
            if (Int16.TryParse(volStr, out volume)) {
                VolumeUtilities.SetApplicationVolume(pid, (float) volume);
            } else {
                Console.WriteLine("SetProgramVolumeCommand: Volume must be a double!");
            }
        } else {
            Console.WriteLine("SetProgramVolumeCommand: PID must be an integer!");
        }
